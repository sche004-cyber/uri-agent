...
Future<AppState> _pumpPostOnboardingApp(WidgetTester tester, {UriClient? client}) async {
...
void main() {
  ...
  group('Profile: role and experience tier (M22.2)', () {
    testWidgets('shows the account role and lets the user change tier', (tester) async {
      ...
      // would hang forever.
      final accepted = await tester.runAsync(() => appState.setExperienceTier('ADVANCED'));
      await tester.pumpAndSettle();
  ...
  group('Profile: devices (M22.2)', () {
    testWidgets('lists this account\'s active devices and marks the current one', (tester) async {
    ...
    testWidgets('the confirm dialog is reachable and revoking updates the list', (tester) async {
      final appState = await _pumpPostOnboardingApp(tester);
      await _openSettingsCategory(tester, 'Profile');
      ...
      expect(find.widgetWithText(ElevatedButton, 'Log out device'), findsOneWidget);
      await tester.tap(find.widgetWithText(TextButton, 'Cancel'));
      await tester.pumpAndSettle();

      // See the tester.runAsync note above - same reason.
      final revoked = await tester.runAsync(() => appState.revokeDevice('mock-device'));
      await tester.pumpAndSettle();
  ...
  group('Capabilities: model status and human-readable labels (M16/M22.3)', () {
    testWidgets('shows model/provider status and a humanized capability name', (tester) async {
      await _pumpPostOnboardingApp(tester);
      await _openSettingsCategory(tester, 'Capabilities');

      expect(find.textContaining('Ollama'), findsOneWidget);
      expect(find.textContaining('qwen3:14b'), findsOneWidget);
  ...
  group('Memory: category picker matches the backend\'s valid vocabulary', () {
    testWidgets('offers exactly the five backend-valid categories, no free text', (tester) async {
      await _pumpPostOnboardingApp(tester);
      await _openSettingsCategory(tester, 'Memory');

      await tester.tap(find.widgetWithText(OutlinedButton, 'Remember something'));
      await tester.pumpAndSettle();
      ...
      for (final label in [
        ...
        // exact count.
        expect(find.text(label), findsWidgets, reason: '$label option missing');
      }
    });
  });

  group('Connections: ADMIN-only actions are protected in the UI (M22.3)', () {
    testWidgets('a non-admin sees a notice and disabled connect/reconnect controls', (tester) async {
      await _pumpPostOnboardingApp(tester, client: _NonAdminMockClient());

      await tester.tap(find.text('Connections'));
      await tester.pumpAndSettle();

      expect(find.textContaining('ADMIN-only action'), findsOneWidget);
    ...
    testWidgets('an admin sees enabled connect/reconnect controls and no notice', (tester) async {
      await _pumpPostOnboardingApp(tester);
