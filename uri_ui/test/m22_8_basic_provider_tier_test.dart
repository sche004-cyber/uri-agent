...
void main() {
  testWidgets('basic provider view never renders token or context window', (tester) async {
    SharedPreferences.setMockInitialValues({});
    tester.view.physicalSize = const Size(1280, 900);
    addTearDown(tester.view.resetPhysicalSize);
    await tester.pumpWidget(AppStateScope(
      state: AppState(client: MockUriClient()),
      child: const MaterialApp(home: ProvidersScreen()),
    ));
    await tester.pumpAndSettle();
    final text = tester.widgetList<Text>(find.byType(Text)).map((item) => item.data ?? item.textSpan?.toPlainText() ?? '').join(' ').toLowerCase();
