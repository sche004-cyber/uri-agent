...
void main() {
  ...
  testWidgets(
    'Test connection uses the typed address even when Save was never pressed',
    (tester) async {
  ...
  testWidgets(
    'a connection failure shows a useful diagnostic reason, not just "Not reachable"',
    (tester) async {
      final client = await pumpSettingsScreen(tester);
  ...
  testWidgets(
    'pressing Save still applies and the badge resets to not-tested-yet',
    (tester) async {
      final client = await pumpSettingsScreen(tester);
  ...
  testWidgets(
    'a saved address survives a simulated relaunch (persistence unaffected by this fix)',
    (tester) async {
