...
void main() {
  ...
  testWidgets(
    'asking from Home surfaces a proposal that can be approved to a result',
    (tester) async {
  ...
  testWidgets('cancelling a proposal reports no action was taken', (
    tester,
  ) async {
    await pumpPostOnboardingApp(tester);
  ...
  testWidgets(
    'a direct question is answered without any proposal or approval step',
    (tester) async {
      await pumpPostOnboardingApp(tester);
  ...
  testWidgets(
    'a request needing a disconnected service is gated, not proposed',
    (tester) async {
      await pumpPostOnboardingApp(tester);
      ...
      expect(
        find.widgetWithText(OutlinedButton, 'Connect Calendar'),
        findsOneWidget,
      );
