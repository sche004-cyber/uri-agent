...
from uri_core.core.skill_evaluator import (
    SkillEvaluator,
...
class TestSkillEvaluatorDirectRegistryEvaluation(unittest.TestCase):
    ...
    def test_directly_relevant_capability_ranks_highly(self):
        evaluator = SkillEvaluator(registry_items=self.registry)
    ...
    def test_evaluator_consumes_context_budget_output(self):
        ...
        self.assertTrue(result["has_suitable_capability"])
        self.assertIn("draft_institutional_note", {c["name"] for c in result["candidates"]})
        self.assertTrue(
    ...
    def test_routing_recommendation_for_optimize_destination(self):
        ...
        evaluator = SkillEvaluator(registry_items=registry)
        result = evaluator.evaluate(
...
class TestSkillEvaluatorClassificationSemantics(unittest.TestCase):
...
class TestSkillEvaluatorEdgeCases(unittest.TestCase):
    """Edge case and defensive behavior tests."""

    def test_empty_registry(self):
        evaluator = SkillEvaluator(registry_items=[])
    ...
    def test_none_intent_handled(self):
        evaluator = SkillEvaluator(registry_items=[uri_core_item("test")])
    ...
    def test_multiple_optimization_skills_distinguished(self):
        ...
        opt_names = [c["name"] for c in result["candidates"] if c.get("category") == "skill_optimization"]
...
if __name__ == "__main__":
    print("=" * 72)
    print("URI SKILL EVALUATOR V1 TEST SUITE")
    print("=" * 72)
    ...
    print()
    print("=" * 72)
    print("SKILL EVALUATOR TESTS:", "PASS" if result.wasSuccessful() else "FAIL")
    print("=" * 72)
