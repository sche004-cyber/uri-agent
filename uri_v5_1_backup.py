...
""".strip()
    ...
    memory_suggestion = ""
    marker = "MEMORY SUGGESTION:"
    if marker in response:
        before, after = response.split(marker, 1)
        response = before.rstrip()
        memory_suggestion = after.strip().splitlines()[0].strip()

    return response, hits, memory_suggestion
...
async function sendMessage() {
  ...
  try {
    ...
    const memoryBox = document.getElementById("memorySuggestion");
    if (data.memory_suggestion) {
      const fact = data.memory_suggestion;
      memoryBox.innerHTML = `
        <div class="src">
          <strong>Memory suggestion</strong><br>
          ${escapeHtml(fact)}<br><br>
          <button class="control" onclick="confirmMemory(${JSON.stringify(fact)})">Save to approved memory</button>
        </div>`;
    } else {
      memoryBox.innerHTML = "";
    }
...
async function confirmMemory(fact) {
  try {
    ...
    const data = await response.json();
    if (data.ok) {
      document.getElementById("memorySuggestion").innerHTML =
        '<div class="src"><strong>Saved.</strong> This fact is now in approved memory.</div>';
    } else {
      addMessage("assistant", "Memory could not be saved: " + (data.error || "unknown error"));
    }
  } catch (error) {
    addMessage("assistant", "Memory save error: " + error);
  }
}
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/chat":
                ...
                answer, sources, memory_suggestion = chat_with_uri(
                    message,
                    history,
                )
